...
`structured_query` は、売上や在庫といった業務データベースに対する集計や問い合わせを担当します。DB 内の複数行を対象とするランキング、平均、件数などは、`structured_query_tool` パイプラインで安全な SQL を生成し、SQLite 上で実行します。

外部データを必要としない一般的な問い合わせは `direct_answer`、非構造化ドキュメントの根拠が必要な問い合わせは `agentic_retrieval` が担当します。業務データの集計は `structured_query_tool` に一元化します。
